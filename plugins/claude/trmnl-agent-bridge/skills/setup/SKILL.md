---
description: Help a user install the trmnl-agent CLI, configure a TRMNL Private Plugin webhook, and verify a safe dry-run payload for Claude Code.
---

# Setup

Use this skill when the user asks to set up TRMNL Agent Bridge for Claude Code.
...
```bash
trmnl-agent sample --source claude | trmnl-agent push --stdin --dry-run
```

6. Only after the dry run succeeds, push a synthetic payload:

```bash
trmnl-agent sample --source claude | trmnl-agent push --stdin
```

From a repository checkout, `examples/sample-payload.json` and `examples/sample-claude-status.json` can also be used for local validation.
